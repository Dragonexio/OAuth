...
1. 访问DragonEx授权登录页面
    - 请求方式：接入方请求DragonEx前端页码，浏览器跳转，GET请求
...
1. 请求登录（获取AccessCode）
    ...
    - url：`https://{host}/api/v1/login/pre/`
    - 传入值：
        ...
        | redirect_url | string | [访问DragonEx授权登录页面][]传入的redirect_url字段 |
    - 返回值data信息：
        
        | 字段名 | 数据类型 | 说明 |
        | --- | --- | --- |
        | redirect_url | string | [拿到AccessCode跳转到接入方的地址][] |
        - redirect_url中带有的参数
    ...
    - 示例
        ...
        ```json
        {
          ...
          "ok": true,
          "Data": {
            "redirect_url": "https://xxx.xxx?app_id=appidfortest&code=b4959e12f0&device=devicefortest&expire_time=1551344583&scopes=1&state=statefortest"
          }
        }
        ```
