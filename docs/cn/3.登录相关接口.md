...
1. 访问DragonEx授权登录页面
    - 请求方式：接入方请求DragonEx前端页面，浏览器跳转，GET请求（DragonEx跳转到接入方时会在url上带上domain字段（`io`、`im`或其他），接入方可根据这个domain字段决定下面登录页面的顶级域名）
...
1. 进入接入方应用的入口页面
    ...
    - 传入值（DragonEx在跳转到App入口页面时会在url的查询参数中附加的字段）：
        ...
        | 字段名 | 数据类型 | 说明 |
        | --- | --- | --- |
        | domain | string | DragonEx用户当前使用的顶级域名解析，如：`io`、`im`、`co` |
