...
1. 退款給用户
    - 请求方式：POST，由接入方服务端调用
    - url：`https://{host}/api/v1/pay/refund/do/`
    - 传入值：

        | 字段名 | 数据类型 | 说明 |
        | --- | --- | --- |
        | ori_trade_no | string | 需要退款订单的转账流水号，要求此订单已付款成功 |
        | refund_trade_no | string | 退款订单的转账流水号，接入方需保证唯一，只允许数字、字母、下划线，最长64个字符，后续可通过此流水号查询退款账单信息 |
        | refund_rate | string | 退款比例，必须处于(0, 1]之间，如需退款的订单支付了100DT，此处值为`"0.66"`，则会退款給用户66个DT |
        | scene | string | 支付场景 |
        | desc | string | 支付描述信息 |
        | device | string | 发起支付的设备信息 |
    - 返回值data信息：
