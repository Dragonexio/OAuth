...
1. 申请支付（用户向接入方支付）
    ...
    - 传入值：
        ...
        | redirect_url | string | 支付成功后，浏览器跳转到的接入方的链接 |
    - 返回值data信息：
    ...
    - 示例
        ...
        ```json
        {
          ...
          "ok": true,
          "Data": {
            "pay_url": "https://oauth.dragonex.im/oauth/pay?app_id=appidfortest&code=c77685c94d&coin_code=usdt&device=deviceForTest&expire_time=1551348353&scene=sceneForTest&state=stateForTest&trade_no=21&volume=1"
          }
        }
        ```
        
        
## DragonEx前端页面请求DragonEx服务端接口

1. 进行支付
    - 请求方式：POST，DragonEx前端调用
    - url：`https://{host}/api/v1/pay/user2app/do/`
    - 传入值：
    ...
    - 返回值data信息：
        ...
        | 字段名 | 数据类型 | 说明 |
        | --- | --- | --- |
        | redirect_url | string | [用户支付成功或失败后跳转到的接入方地址][] |
        - redirect_url中带有的参数
    ...
    - 示例
        ...
        ```json
        {
          ...
          "ok": true,
          "Data": {
            "redirect_url": "https://xxx.xxx?app_id=&coin_code=usdt&direction=1&state=stateForTest&status=3&trade_no=22&uid=1000000&volume=1&cut_volume=0"
          }
        }
        ```
...
1. 用户支付成功或失败后跳转到的回调地址
    ...
    - 传入值（通过url查询参数传递）：
        ...
        | direction | int | 订单方向 |
        | status | int | [订单状态][payorderstatus] |
    - 返回值data信息：无，返回200状态码即认为回调成功，若不成功在36H内每隔10min重试一次
...
1. 查询订单历史记录
    ...
    - 传入值：
        ...
        | start_time | int | 查询起始时间，传0不做起始时间限制 |
        | end_time | int | 查询结束时间，传0不做结束时间限制 |
        | offset | int | 偏移 |
